...
def _detect_fw(root):
    for sub in ['', 'app', 'server', 'backend']:
        ...
        if os.path.isfile(os.path.join(root, sub, 'build.gradle')) or os.path.isfile(os.path.join(root, 'build.gradle')):
            return 'spring'
    # Prefer Python framework detection over Node: a root package.json may be a
    # legacy/leftover scaffold (e.g. an earlier Express prototype) sitting next
    # to the actual active Python backend.
    ...
    for r, _, fls in os.walk(root):
        norm_r = r.replace('\\', '/')
        if any(x in norm_r for x in ['/__pycache__/', '/venv/', '/.venv/', '/.git/', '/node_modules/']): continue
        for f in fls:
            if f.endswith('.py'):
                try:
...
_FASTAPI_EXCLUDE = ['/__pycache__/', '/venv/', '/.venv/', '/env/', '/.git/', '/tests/', '/test/', '/node_modules/']
_GENERIC_ROUTE_FILENAMES = {'router', 'routes', 'urls', 'views', 'api', 'endpoints', 'controller'}
...
def _scan_fastapi(root):
    ...
    for r, _, fls in os.walk(root):
        norm_r = r.replace('\\', '/')
        if any(x in norm_r for x in _FASTAPI_EXCLUDE):
            continue
        for f in fls:
            if f.endswith('.py'):
                fp = os.path.join(r, f)
                ...
                # Aggregator pattern: api_router = APIRouter(prefix="/api/v1") with no name
                # match on the router-variable side — used as a fallback base prefix for
                # modules whose own APIRouter() declares no prefix of its own.
                gm = re.search(r"\b(?:api_router|app_router|root_router)\s*=\s*APIRouter\([\s\S]*?prefix=['\"]([^'\"]+)['\"]", txt)
                if gm and not global_prefix:
    ...
    for idx, rf in enumerate(sorted(set(route_files))):
        ...
        fn = os.path.basename(rf)
        raw = re.sub(r'\.py$', '', fn)
        # Generic filenames (router.py, routes.py, ...) collide across modules —
        # fall back to the parent folder name so each feature module stays distinct.
        raw_id_source = raw
        if raw.lower() in _GENERIC_ROUTE_FILENAMES:
            parent = os.path.basename(os.path.dirname(rf))
            if parent and parent.lower() not in _GENERIC_ROUTE_FILENAMES:
                raw_id_source = parent
        name = raw_id_source.replace('_', ' ').title()
        mid = raw_id_source.lower().replace('-', '_')

        pm = re.search(r"APIRouter\([\s\S]*?prefix=['\"]([^'\"]+)['\"]", txt)
        if pm:
            bp = pm.group(1)
        elif mid in app_prefixes:
            bp = app_prefixes[mid]
        elif 'APIRouter(' not in txt:
            # No router object here at all (e.g. @app.get(...) declared straight on
            # the FastAPI() instance) — these aren't mounted under the aggregator's
            # prefix, so don't inherit it.
            bp = ''
        elif global_prefix:
            bp = global_prefix
        else:
            bp = f"/api/{raw_id_source.replace('_', '-')}"
        ...
        if eps:
            modules.append({
                ...
                'color': _color(raw_id_source, idx),
                'icon': _icon(raw_id_source),
                'files': [fn],
                'permissions': [],
                'endpoints': eps
            })
