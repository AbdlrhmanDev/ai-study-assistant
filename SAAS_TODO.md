...
- [x] Add email verification. `email_verified_at` on `users`; `email_verification_tokens` (SHA-256-hashed opaque token, same pattern as session tokens); `POST /auth/verify-email/request` (authenticated resend) and `POST /auth/verify-email/{token}` (public, rate-limited). Sent automatically on registration, best-effort (a send failure logs a warning and never blocks signup). Changing your email clears verification on the new address. Frontend: `/verify-email/[token]` auto-verifies on load; Settings shows an unverified banner with a resend button. Verified live against the real DB 2026-08-12 (register → verify → reuse-rejected).
- [x] Add forgot-password and secure password-reset flows. `password_reset_tokens` (single-use via `used_at`, not deleted, for an audit trail); `POST /auth/forgot-password` always responds identically whether or not the email exists **and regardless of whether the send itself succeeds** (a caught bug: an unhandled send failure would have leaked account existence via a 202-vs-500 status difference -- fixed by swallowing send failures there specifically, unlike the verification resend above which surfaces failures since there's no enumeration risk on an authenticated action). `POST /auth/reset-password` validates the token, sets the new password, and revokes every existing session (no current session to preserve, unlike an authenticated password change). Frontend: `/forgot-password`, `/reset-password/[token]`, a "Forgot your password?" link on `/login`. Verified live against the real DB 2026-08-12 (forgot → reset → reuse-rejected → password hash actually changed → nonexistent-email case raises nothing).
...
- [x] Invalidate relevant sessions after password and security changes. Change-password deletes every other session for that user; password reset deletes every session (no current one to keep).
- [x] Add transactional email delivery. `core/mail.py` is a generic `send_email`/`send_email_sync`, extracted from the SMTP client that previously lived only inside `growth/reminders.py` (which now calls the shared version) -- reused by verification and reset-password emails. Deliverability monitoring itself (bounces, provider-side metrics) is still out of scope: no transactional email *provider* is configured, only raw SMTP.
- [x] Create email templates for verification, password reset, security notices, and account deletion. Verification and reset-password emails now exist (plain-text, generated inline in `auth/service.py`, matching the existing reminder email's style rather than an HTML template system). Security-notice and account-deletion emails remain unwritten -- no send path calls them yet.
...
- [x] Build evaluation datasets for citation and answer quality. `docs/rag-evaluation.jsonl` is now a real 14-query hand-labeled dataset (2 queries per note, phrased/paraphrased differently) over the demo seed's 7-note "Biology Foundations" topic. `scripts/seed_demo.py` extended from a single note to 7 distinct-but-related concepts (photosynthesis, cellular respiration, mitosis, DNA replication, osmosis/diffusion, enzymes, homeostasis) so retrieval has to actually discriminate, and now indexes every seeded note immediately instead of leaving it unindexed. `scripts/evaluate_rag.py` resolves topic/source by title instead of hardcoded ids (stable across environments). Verified against the live database 2026-08-12: `recallAtK: 0.8571, mrr: 0.8214` over 14 examples -- a real, non-trivial signal (not 1.0), confirming the eval set has discriminative power. `docs/rag-evaluation.example.jsonl` updated to the same title-based schema.
...
| 2026-08-12 | Phase 3 RAG evaluation dataset (closing the one remaining gap from the two entries above) | Ran live end-to-end against the real Neon database (after applying 2 pending migrations, `add plan to users` and `add status to exams`, that the DB was missing): `python scripts/seed_demo.py` then `python scripts/evaluate_rag.py docs/rag-evaluation.jsonl` returned `{"examples": 14, "recallAtK": 0.8571, "mrr": 0.8214}`. | `scripts/seed_demo.py` extended from one note to 7 distinct concepts under "Biology Foundations" (photosynthesis, cellular respiration, mitosis, DNA replication, osmosis/diffusion, enzymes, homeostasis), each now indexed immediately on seed (previously the single seeded note was never indexed at all, so it wasn't retrievable). `docs/rag-evaluation.jsonl` is a real 14-query hand-labeled dataset, 2 phrased/paraphrased queries per note. `scripts/evaluate_rag.py` rewritten to resolve topic/source by title instead of hardcoded numeric ids, so the dataset stays valid across environments. The confirmed feedback route-shadowing bug and AI-artifact caching noted as gaps in the 2026-08-12 entry above were re-checked this session and found already fixed/wired in the current working tree -- `ai/service.py::submit_message_feedback` already forwards `comment` to `growth.service.rate_answer` on the canonical route, and `cache_artifact`/`get_cached_artifact` are already wired into quiz/exam/flashcard/mind-map generation. |
| 2026-08-12 | `product_events.properties` JSON -> JSONB fix (minor consistency fix found during the RAG eval verification pass) | Verified directly against the live database: `SELECT '{}'::json = '{}'::json` reproducibly errors (`operator does not exist: json = json`, plain Postgres `json` has no equality operator); after the migration, `SELECT '{}'::jsonb = properties FROM product_events` succeeds and `pg_typeof(properties)` returns `jsonb`; `alembic check` reports "No new upgrade operations detected" (model/DB back in sync). | `growth/model.py::ProductEvent.properties` was the only JSON-typed column in the codebase still using plain `JSON` instead of `JSONB` (the other 7 JSON columns -- exams, workspace, quizzes, cleanup, auth, jobs, mind_map -- already used `JSONB`). Added hand-written migration `d3eb5af2dc34` (`ALTER COLUMN ... USING properties::jsonb`) rather than relying on autogenerate, which can omit the `USING` cast on a non-empty table. Not an active CI break (`alembic check` passed both before and after), just an inconsistency worth closing since a schema change was already in flight this session. |
| 2026-08-12 | Account lifecycle: email verification + forgot/reset password (closes the last 2 unchecked items in this section) | Ran live end-to-end against the real Neon database with a disposable test account, fresh DB session per step (matching real per-request semantics): register -> auto-sent verification (send fails cleanly without SMTP configured, doesn't block signup) -> verify -> reused verification token correctly rejected -> forgot-password -> reset -> password hash actually changed -> reused reset token correctly rejected -> forgot-password for a nonexistent email raises nothing (no status-code enumeration signal). `tsc --noEmit` and `eslint` clean on every new/changed frontend file (only pre-existing unrelated errors remain elsewhere). Migration `0e6bfb098e7e` applied (`users.email_verified_at`, `email_verification_tokens`, `password_reset_tokens`); `alembic check` reports back in sync afterward. | New: `core/mail.py` (generic `send_email`/`send_email_sync`, extracted from `growth/reminders.py`'s SMTP client, which now calls the shared version unchanged in behavior); `core/security.py::new_opaque_token`/`hash_opaque_token` (public wrappers around the existing session-token hashing, reused instead of duplicated); `auth/model.py::EmailVerificationToken`/`PasswordResetToken`; four new `/auth` endpoints (`verify-email/request`, `verify-email/{token}`, `forgot-password`, `reset-password` -- static-route-before-dynamic-route ordering double-checked via the OpenAPI schema); frontend `/forgot-password`, `/reset-password/[token]`, `/verify-email/[token]`, a "Forgot your password?" link on `/login`, and an unverified-email banner with a resend button in Settings. **Bug caught and fixed before it shipped:** `request_password_reset` originally let a send failure propagate uncaught -- since it only calls `send_email` when the account exists, an SMTP outage (or simply `SMTP_HOST` unset, the actual state of this environment) would have made `/auth/forgot-password` return 202 for unregistered emails but 500 for registered ones, leaking account existence through the status code alone. Fixed by catching and logging instead of raising, specifically only on that path (the authenticated verification-resend deliberately keeps raising, since there's no enumeration risk there and the user deserves to know delivery failed). Changing your email now also clears `email_verified_at` on the new address. |
